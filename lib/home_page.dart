...
class _HomePageState extends State<HomePage> {
  var myInitialItem = 'Sugar';
  List<String> myItems = [
    'Sugar',
    'Water',
    'Milk',
  ];
  var myInitialCurrency = 'Ethiopian Birr';
  ...
  var myInitialMetric = 'Kilogram';
  List<String> myMetric = [
    'Kilogram',
    'Gram',
    'Liter',
    'Mililiter',
                    ...
                    children: [
                      Padding(
                          ...
                          child: DropdownButton(
                            onChanged: (value) {
                              setState(() {
                               myInitialCurrency = value;
                              });
                              log(myInitialCurrency);
                            },
                            value: myInitialCurrency,
                            items: myCurrency.map((item) {
                              return DropdownMenuItem(
                                  value: item, child: Text(item));
                            }).toList(),
                          ) //dropdownButton
                      ...
                      Padding(
                          ...
                          child: DropdownButton(
                            onChanged: (value) {
                              setState(() {
                               myInitialMetric = value;
                              });
                              log(myInitialMetric);
                            },
                            value: myInitialMetric,
                            items: myMetric.map((item) {
                              return DropdownMenuItem(
                                  value: item, child: Text(item));
                            }).toList(),
                          ) //dropdownButton
  ...
  void _onClickSave() async {
    if (_file != null) {
      ...
      if (nameController.text?.isEmpty ?? true) {
        ...
        await data.writeAsString('${timestamp}, ${myInitialItem},${myInitialCurrency} ${priceController.text},${quantityController.text} ${myInitialMetric}\n',mode: FileMode.append);
      } else {
